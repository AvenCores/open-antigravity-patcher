...
const (
	...
	AgRegistrySubkey             = `SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall\{AA73B3E3-C6C8-45C8-B1DC-4AE56C751432}_is1`
	IntegrityBlockSize           = 4 * 1024 * 1024
)

var RuntimeExperimentsToDisable = []string{
...
const (
	...
	ColorGray    = CSI + "90m"
	ColorWhite   = CSI + "97m"
	ColorMagenta = CSI + "35m"
)

var (
	ReAuthIsGoogleInternal = regexp.MustCompile(
		`if\(\s*((?:this\.[A-Za-z_$][\w$]*\.send\(\{type:[^}]+\}\)\s*,\s*)?this\.[A-Za-z_$][\w$]*\.resetIsTierGCPTos\(\)\s*,\s*)this\.[A-Za-z_$][\w$]*\.isGoogleInternal\s*\)`,
	)
	ReAuthIsGoogleInternalOld = regexp.MustCompile(
		`if\(\s*(this\.[A-Za-z_$][\w$]*\.resetIsTierGCPTos\(\)\s*,\s*)this\.[A-Za-z_$][\w$]*\.isGoogleInternal\s*\)`,
	)
	ReAuthIsGoogleInternalNew = regexp.MustCompile(
		`if\(\s*(this\.[A-Za-z_$][\w$]*\.send\(\{type:[^}]+\}\)\s*,\s*this\.[A-Za-z_$][\w$]*\.resetIsTierGCPTos\(\)\s*,\s*)this\.[A-Za-z_$][\w$]*\.isGoogleInternal\s*\)`,
	)
)

var PackExcludePaths = map[string]bool{
	"downloaded_frontend_main.js": true,
	"frontend_patch_result.json":  true,
	"dist/main.js.bak":            true,
}

const LocalPatchServerKey = `-----BEGIN RSA PRIVATE KEY-----
...
const LocalPatchServerCert = `-----BEGIN CERTIFICATE-----
...
const AntigravityInjectionCodeTemplate = `
