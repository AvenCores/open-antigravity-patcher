...
1. В функции `handleAuthResult` (или `handleAuthResult.func1`) проверяется поле `AuthResult.hasValidAuth` (байт по смещению `+8`):
   ```asm
   cbnz x1, error_path   ; e1 20 00 b5
   cbz  x0, failure_path ; 80 15 00 b4
   ldrb w3, [x0, #8]     ; 03 20 40 39     <-- загрузка hasValidAuth
   tbnz w3, #0, success  ; 43 15 00 37     <-- проверка eligibility
   ```
2. Патчер находит эту последовательность по сигнатуре и заменяет инструкцию `ldrb w3, [x0, #8]` на `mov w3, #1` (`23 00 80 52`). В результате регистр `w3` всегда равен 1, и условный переход `tbnz` всегда переходит на success-путь, минуя экран блокировки.
