...
1. В функции `handleAuthResult` проверяется поле `ServerBackend+0x1b0` (ineligibility-объект):
   ```asm
   ldr  x3, [x0, #0x1b0] ; 03 d8 40 f9     <-- загрузка eligibility объекта
   cbz  x3, skip         ; xx xx xx b4     <-- переход если NULL (пропуск экрана)
   mov  x0, x3           ; e0 03 03 aa
   ```
2. Патчер заменяет инструкцию `ldr x3, [x0, #0x1b0]` на `mov x3, xzr` (`e3 03 1f aa`). Благодаря этому `cbz` всегда считает регистр нулевым и переходит на `skip`, минуя вызов ineligibility-экрана.
